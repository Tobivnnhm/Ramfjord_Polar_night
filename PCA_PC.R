...
setwd(file.path(res))
df <- read.csv("MS23_result_pksReduced_PC.csv", header=TRUE, stringsAsFactors=FALSE)
head(df)
...
setwd(file.path(res))
write.csv(scree.data, "MS23_PC_pca_scree.csv", row.names = TRUE)
write.csv(score.data, "MS23_PC_pca_scores.csv", row.names = TRUE)
write.csv(loadings.data, "MS23_PC_pca_loadings.csv", row.names = TRUE)
...
indmeta <- merge(meta, data, by.x="sample_name", by.y="row.names")
data <- indmeta
...
var1
# Set color code
data$season <- factor(data$season, levels = c("september", "october", "november", "december", "january", "february"))
group_color <- paletteer_d("LaCroixColoR::PassionFruit", n=6) # or inferno
names(group_color) <- c("september", "october", "november", "december", "january", "february")
...
g1 + geom_label_repel(data=data %>% filter(PC2 < -2 & season == "september"), aes(label=sample_code), show.legend = TRUE, force = 4, label.padding = 0.1) 

# save as png file
png(file="MS23_PC_scores.plot_PC12.png", height=2400, width=2800, res=350)
g1
dev.off()
...
g2 <- ggplot(data, aes(PC1, PC3)) +
  ...
  my.theme +
  theme(legend.position= "right", legend.title = element_text("Season"))
png(file="MS23_PC_scores.plot_PC13.png", height=2400, width=2800, res=350)
g2 
dev.off()
...
g3 <- ggplot(data, aes(PC2, PC3)) +
  ...
  my.theme +
  theme(legend.position= "right", legend.title = element_text("Season"))
g3 + geom_label_repel(data=data %>% filter(PC2 < -1 & season == "november"), aes(label=sample_code), show.legend = TRUE, force = 4, label.padding = 0.1) 
png(file="MS23_PC_scores.plot_PC23.png", height=2400, width=2800, res=350)
g3 
dev.off()
g1
g2
...
png(file="MS23_PC_loadings.plot_PC1PC2.png", height=2400, width=2800, res=350)
g1a
dev.off()
g2a <- 
  g2 +
...
write.csv(loadings.sig, "MS23_PC_sig_loadings_PC123.csv")
...
write.csv(pca.sig.vars, "MS23_PC_dat_sig_loadings.csv")
