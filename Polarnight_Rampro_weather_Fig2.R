...
plot(Data$Temp_RF ~ Data$day, type="l", axes=F, 
     xlab="Date", ylab="Mean daily temperature [°C]", lwd=1.5)
...
points(Data$prec ~ Data$day, type="l", 
      lwd=1.5, col="grey")
axis(2, las=1)
axis(4, at=c(0,5,10,15),labels=FALSE)
mtext(c("0","50","100","150"),4, line=0.5,cex=1, at=c(0,5,10,15)) 
...
points(c(6,20,62,82,111,139,174,193), c(rep(0,8)), pch=21, bg="red", cex=2)
legend(125,20, c("Precipitation Tromsø", "Snow depth Tromsø","Sampling time"), 
       col=c("grey","grey","black"), pch=c(23, 23, 21), pt.bg=c(rgb(0, 0, 0.75, 0.15),"lightgrey","red"))
title(main="B",adj=0, cex=0.75)

plot(Data$maxwind ~ Data$day, type="n", axes=F, 
     xlab="Date", ylab="Wind speed [m/s]", ylim=c(0,12.8))
...
legend(1,12.5, c("Average wind speed", "Maximum wind speed","Sampling time"), 
       col=c("black","grey","black"), pch=c(23, 23, 21), pt.bg=c("grey",rgb(1, 0, 0, 0.3),"red"))
...
plot(Data2$PAR ~ Data2$day, type="n", axes=F, 
     xlab="Date", ylab=expression(paste("Average daily Light  PAR  [ ",mu,"E m"^"-2","s"^"-1","]")))
...
legend(70,12.5, c("PAR Olavsvern", "PAR at 5m depth","Sampling time"), 
       col=c("black","black","black"), pch=c(23, 25, 21), pt.bg=c("gold","yellow","red"))
